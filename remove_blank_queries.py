...
def rewrite_input_file():
    ...
    for line in f:
        ...
        # Remove duplicates from each attribute set.
        disease_list = list(set(diseases.strip().split(':')[:-1]))
        symptom_list = list(set(symptoms.strip().split(':')[:-1]))
        herb_list = list(set(herbs.strip().split(':')[:-1]))
        ...
        visit_line = '%s\t%s\t%s\t%s\t%s\t%s\n' % (':'.join(disease_list),
            name, dob, visit_date, ':'.join(symptom_list),
            ':'.join(filtered_herb_list))
